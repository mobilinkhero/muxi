...
@section('title', 'Modify Brief - Admin')

@section('content')
    <div class="h-reveal" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3rem;">
        <div>
            <h1 style="font-weight: 900; font-size: 2.5rem; letter-spacing: -1px; margin: 0;">Modify Brief</h1>
            <p style="color: #94A3B8; margin-top: 0.5rem;">Editing intelligence record: {{ $post->title }}</p>
        </div>
            ...
            <div class="h-card">
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="h-input" required value="{{ old('title', $post->title) }}">
                    </div>

                    <div class="form-group">
                        <label class="h-label">Slug Identifier (URL)</label>
                        <input type="text" name="slug" class="h-input" required value="{{ old('slug', $post->slug) }}"
                ...
                <div class="form-group mb-4">
                    ...
                    <div style="display: flex; align-items: flex-start; gap: 1.5rem;">
                        @if($post->image_url)
                            <div
                                style="border: 1px solid var(--h-border); border-radius: 8px; padding: 4px; background: rgba(0,0,0,0.3);">
                                <img src="{{ asset($post->image_url) }}"
                                    style="max-width: 150px; border-radius: 4px; display: block;">
                            </div>
                        @endif
                        <div style="flex: 1;">
                            <label class="btn-primary-h"
                                ...
                                <i class="fas fa-upload"></i> Update Cover Image
                                <input type="file" name="image" accept="image/*" style="display: none;"
                                    onchange="document.getElementById('fileName').innerText = this.files[0].name">
                            </label>
                            <div id="fileName"
                                style="margin-top: 0.5rem; font-size: 0.8rem; color: #10B981; text-align: center;"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea name="content" class="h-input" rows="15" required
                        style="font-family: 'JetBrains Mono'; line-height: 1.6;">{{ old('content', $post->content) }}</textarea>
                ...
                <div
                    ...
                    <label style="display: flex; align-items: center; gap: 12px; cursor: pointer;">
                        <input type="checkbox" name="is_published" value="1" id="is_published" {{ $post->is_published ? 'checked' : '' }} style="width: 20px; height: 20px; accent-color: var(--h-primary);">
                        <div>
                            <div style="font-weight: 800; color: white;">ACTIVE BROADCAST</div>
                            <div style="font-size: 0.75rem; color: #94A3B8;">Visible on public channels</div>
                        </div>
                    </label>

                    <button type="submit" class="btn-primary-h">
                        <i class="fas fa-save"></i> Update Briefing
                    </button>
                </div>
            </div>
        </form>
